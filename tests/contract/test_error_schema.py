...
def test_invalid_op_returns_error_schema(tmp_path):
    ...
    res = client.post("/environments/dev/extensions/openehr.rps_dual/does_not_exist", json={})
...
def test_unknown_strategy_returns_not_found(tmp_path):
    app = create_app(str(tmp_path / "reg.json"))
    client = TestClient(app)
    res = client.get("/strategies/unknown")
...
def test_internal_error_returns_500(tmp_path, monkeypatch):
    ...
    res = client.post("/environments/dev/query", json={"domain": "openEHR", "query": {"scope": "patient", "predicates": []}})
