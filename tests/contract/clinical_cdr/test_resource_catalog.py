...
def test_catalog_lists_only_the_active_strategy_scope_without_database_access():
    ...
    assert catalog["storage_schema_version"] == "1"
    ...
    assert catalog["schema_resource_count"] > catalog["resource_count"]
    assert {item["resource_type"] for item in catalog["resources"]} >= {
        "Patient",
        "Observation",
        "Condition",
    }
    assert all(item["capabilities"]["storable"] for item in catalog["resources"])
...
def test_catalog_rejects_schema_types_outside_the_strategy_scope():
    with pytest.raises(KehrnelError) as exc_info:
        fhir_resource_catalog(_ctx(), {"resource_type": "CapabilityStatement"})

    assert exc_info.value.code == "FHIR_RESOURCE_NOT_IN_STRATEGY_SCOPE"
