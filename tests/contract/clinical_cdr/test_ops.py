"""Contract tests for fhir.clinical_cdr search ops (prompt 11)."""
...
from kehrnel.engine.strategies.fhir.clinical_cdr import stats as fhir_stats_mod
...
async def test_fhir_search_compile_and_execute():
    ...
    with patch(
        "kehrnel.engine.strategies.fhir.clinical_cdr.query.compile_fhir_query",
        new_callable=AsyncMock,
        return_value=plan,
    ) as compile_mock:
        with patch(
            "kehrnel.engine.strategies.fhir.clinical_cdr.query.execute_fhir_query",
            new_callable=AsyncMock,
            return_value=type(
                "R",
                (),
...
async def test_fhir_stats_dry_structure():
    ...
    with patch(
        ...
        with patch(
            "kehrnel.engine.strategies.fhir.clinical_cdr.stats.bridge.known_generation_resource_types",
