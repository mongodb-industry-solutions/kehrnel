...
`fhir.rps_canonical` compiles **FHIR REST search parameters** into **MongoDB aggregation pipelines** using **fhir-mql** (`fhir_search_to_mql.FHIRSearchConverter`).
...
Contract tests in `tests/contract/fixtures/fhir_golden_queries.json` cover compile (and execute when `FHIR_CONTRACT_MONGO=1`) for Patient, Schedule, Slot, and Appointment searches.
