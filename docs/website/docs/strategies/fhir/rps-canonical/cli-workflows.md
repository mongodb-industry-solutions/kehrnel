...
# FHIR RPS Canonical CLI Workflows

End-to-end path for `fhir.rps_canonical` on a local Kehrnel runtime.

1. Start Kehrnel and install the `[fhir]` extra
2. Activate `fhir.rps_canonical` on an environment
...
Packaged specification and API samples live under `src/kehrnel/engine/strategies/fhir/rps_canonical/specification/` in the kehrnel repository.
...
Small batch (`src/kehrnel/engine/strategies/fhir/rps_canonical/specification/job_generate_small.json`):
...
Requires active `fhir.rps_canonical` on the environment. Pass `x-active-env` (or configured default):
