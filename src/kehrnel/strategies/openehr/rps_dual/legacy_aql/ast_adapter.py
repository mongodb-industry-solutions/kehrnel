...
def _qualify_path(path: str, ehr_alias: str, comp_alias: str) -> str:
    """
    The legacy transformers expect alias-qualified paths (e/ehr_id/value, c/...).
    """
...
def _adapt_where(ir: AqlQueryIR, ehr_alias: str, comp_alias: str) -> Dict[str, Any] | None:
    ...
    # Always wrap in logical AND so the legacy builders take the logical path (needed for $all and compound.filter).
...
def adapt_ir_to_legacy_ast(ir: AqlQueryIR, ehr_alias: str = "e", composition_alias: str = "c") -> Dict[str, Any]:
    ast: Dict[str, Any] = {}
    ast["from"] = {"rmType": "EHR", "alias": ehr_alias}
    # Emit a contains tree with aliases so legacy ContextMapper can resolve node scopes.
