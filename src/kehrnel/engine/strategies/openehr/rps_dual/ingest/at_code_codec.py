...
async def load_codes_from_db(db, config: dict) -> None:
    ...
    # Load at_codes (allow non-int codes too)
    at_book = {}
    ...
    with CACHE_LOCK:
        ...
        # keep numeric _min if present, otherwise retain existing
        min_val = doc.get("_min", SEQ.get("at", -1))
...
def get_reverse_mappings():
    ...
    with CACHE_LOCK:
        ...
        at_reverse = {v: k for k, v in CODE_BOOK["at"].items()}
