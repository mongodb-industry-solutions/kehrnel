...
SUPPORTED_RELEASES = frozenset({"R5", "R6"})
...
def _base_schema(release: str) -> dict[str, Any]:
    normalized = str(release or "").strip().upper()
    if normalized not in SUPPORTED_RELEASES:
        raise ValueError(f"Unsupported FHIR release: {release!r}")
...
def schema_resource_types(release: str) -> frozenset[str]:
    ...
    definitions = _base_schema(_normalized_release(release)).get("definitions") or {}
    return frozenset(
        name
        for name, definition in definitions.items()
        if isinstance(definition, dict)
        and isinstance(definition.get("properties"), dict)
        and "resourceType" in definition["properties"]
    )
...
def validate_resource(
    ...
    *,
    ...
    if not isinstance(resource, dict):
        return None, [_finding(index=index, code="FHIR_RESOURCE_NOT_OBJECT", message="FHIR resource must be a JSON object")]
    ...
    if not isinstance(resource_type, str) or not RESOURCE_TYPE.fullmatch(resource_type):
        findings.append(_finding(index=index, code="FHIR_RESOURCE_TYPE_INVALID", message="resourceType is required and must be a FHIR type name"))
    elif supported_resource_types is not None and resource_type not in supported_resource_types:
    ...
    if not isinstance(resource_id, str) or not FHIR_ID.fullmatch(resource_id):
        findings.append(_finding(index=index, code="FHIR_ID_INVALID", message="id is required and must match [A-Za-z0-9-.]{1,64}", resource_type=resource_type if isinstance(resource_type, str) else None))
    unsafe = _unsafe_key_path(resource)
    if unsafe:
        findings.append(_finding(index=index, code="FHIR_UNSAFE_KEY", message="FHIR JSON contains a MongoDB-unsafe property name", resource_type=resource_type if isinstance(resource_type, str) else None, resource_id=resource_id if isinstance(resource_id, str) else None, path=unsafe))
    if client_operational:
        findings.append(_finding(index=index, code="FHIR_OPERATIONAL_FIELDS_REMOVED", message=f"Client operational fields were removed: {', '.join(client_operational)}", severity="warning", resource_type=resource_type if isinstance(resource_type, str) else None, resource_id=resource_id if isinstance(resource_id, str) else None))
    ...
    if level == "base":
        if release not in SUPPORTED_RELEASES:
            findings.append(_finding(index=index, code="FHIR_BASE_VALIDATION_UNAVAILABLE", message=f"Base JSON Schema validation is not bundled for {release}", resource_type=resource_type, resource_id=resource_id))
        else:
            validator = _base_validator(release, resource_type)
            if validator is None:
                findings.append(_finding(index=index, code="FHIR_RESOURCE_TYPE_UNKNOWN", message=f"{resource_type} is not present in the bundled {release} schema", resource_type=resource_type, resource_id=resource_id))
            else:
                errors = sorted(validator.iter_errors(resource), key=lambda error: list(error.absolute_path))
                for error in errors[:max_schema_findings]:
                    dotted = ".".join(str(part) for part in error.absolute_path) or "$"
                    findings.append(_finding(index=index, code="FHIR_BASE_SCHEMA_INVALID", message=error.message, resource_type=resource_type, resource_id=resource_id, path=dotted))

    return resource, findings


def available_validation_levels(release: str) -> tuple[str, ...]:
    _normalized_release(release)
...
def validate_level(level: str, release: str) -> str:
    ...
    available_validation_levels(release)
    return normalized
