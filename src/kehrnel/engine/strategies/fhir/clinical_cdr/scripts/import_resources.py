...
def _check_canceled(should_cancel: CancelCallback | None) -> None:
    if should_cancel and should_cancel():
        raise KehrnelError(
            code="JOB_CANCELED", status=499, message="FHIR import canceled by user"
        )
...
def parse_import_payload(
    ...
    if payload.get("ndjson") is not None:
        ...
        if not isinstance(text, str):
            raise KehrnelError(
                code="INVALID_INPUT", status=400, message="ndjson must be a string"
            )
        resources: list[Any] = []
    ...
    if source is not None:
        if not isinstance(source, dict) or source.get("resourceType") != "Bundle":
            raise KehrnelError(
                code="INVALID_INPUT",
                status=400,
                message="bundle must be a FHIR Bundle resource",
            )
    ...
    if payload.get("resources") is not None:
        ...
        if not isinstance(resources, list):
            raise KehrnelError(
                code="INVALID_INPUT", status=400, message="resources must be an array"
            )
...
async def fhir_import_resources(
    ...
    *,
    ...
    try:
        ...
        raise KehrnelError(
            code="FHIR_VALIDATION_LEVEL_UNAVAILABLE", status=400, message=str(exc)
        ) from exc

    # Persistence is independent from example generation recipes.  The mandatory
    # projection contract still requires an active fhir-mql mapping, so adding a
    # reviewed/generated mapping automatically expands the write surface.
    denormalizer = _build_denormalizer(cfg)
    capability_sets = resolve_resource_capabilities(cfg, denormalizer.config_loader)
    supported_resource_types = set(capability_sets.storable)
    ...
    if len(resources) > max_resources:
        raise KehrnelError(
            ...
            details={
                ...
                "max_resources_per_request": max_resources,
            },
        )
    ...
    if mode not in {"upsert", "create"}:
        raise KehrnelError(
            code="INVALID_INPUT",
            status=400,
            message="mode must be 'upsert' or 'create'",
        )
    ...
    await _emit(
        ...
        5,
        "validating",
        {"received": len(resources), "source_format": source_format},
    )
    valid: list[dict[str, Any]] = []
    valid_input_indexes: list[int] = []
    ...
    for index, raw in enumerate(resources):
        ...
        if resource is None or any(
            item["severity"] == "error" for item in item_findings
        ):
            continue
        ...
        valid_input_indexes.append(index)

    profile_report = await profile_validation.validate_profiles(
        ctx, cfg, valid, resource_indexes=valid_input_indexes
    )
    findings.extend(profile_report["findings"])
    failed_profile_indexes = set(profile_report["failed_resource_indexes"])
    if failed_profile_indexes:
        retained = [
            (input_index, resource)
            for input_index, resource in zip(valid_input_indexes, valid, strict=True)
            if input_index not in failed_profile_indexes
        ]
        valid_input_indexes = [item[0] for item in retained]
        valid = [item[1] for item in retained]
    errors = [
        item
    ...
    report: dict[str, Any] = {
        ...
        "resource_support_source": (
            "provisional-r4-reviewed-contract"
            if release == "R4"
            else "active-release-schema-and-fhir-mql-config"
        ),
        "validation": {
            "level": level,
            "profile_conformance": bool(
                profile_report["enforced"] and profile_report["failed"] == 0
            ),
            "profile_validation": profile_report,
            "received": len(resources),
            "valid": len(valid),
            "invalid": len(resources)
            - len(valid)
            + len(
                [
                    f
                    for f in findings
                    if f.get("code", "").startswith("FHIR_NDJSON")
                    or f.get("code") == "FHIR_BUNDLE_ENTRY_INVALID"
                ]
            ),
            "findings": findings,
        },
        "resource_counts": dict(
            sorted(Counter(str(resource["resourceType"]) for resource in valid).items())
        ),
    }
    ...
    report["document_contract"] = {
        ...
        "resource_projection_versions": dict(
            sorted(versions.resource_projection_versions.items())
        ),
    }
    ...
    if dry_run:
        ...
        await _emit(
            progress_cb, 100, "completed", {"dry_run": True, "valid": len(valid)}
        )
        return report
    ...
    index_result = await indexes.fhir_ensure_indexes(
        ctx, {"resource_types": searchable_types}
    )
    ...
    try:
        ...
        await _emit(
            ...
            70,
            "persisting",
            {"valid": len(projected), "database": database},
        )
        write_result = await asyncio.to_thread(
            adapter.persist_many, projected, mode=mode
        )
    except Exception as exc:
        if exc.__class__.__name__ in {"BulkWriteError", "DuplicateKeyError"}:
            raise KehrnelError(
                code="FHIR_WRITE_CONFLICT",
                status=409,
                message="A FHIR logical id already exists",
                details={"mode": mode},
            ) from exc
        raise
    finally:
    ...
    report["collections"] = [
        bridge.collection_name(prefix, resource_type)
        for resource_type in sorted(report["resource_counts"])
    ]
    report["committed"] = True
    report["ok"] = not errors

    await _emit(
        progress_cb, 100, "completed", {"written": write_result.get("processed", 0)}
    )
    return report
