...
from kehrnel.engine.strategies.fhir.rps_canonical._paths import FHIR_GEN_ROOT
...
from kehrnel.engine.strategies.fhir.rps_canonical.scripts import watermark
...
async def synthetic_generate_batch(
    ...
    *,
    ...
    See ``specification/README.md`` in this strategy pack for the payload contract.
    """
    cfg = bridge.resolve_strategy_config(ctx)
    requested = bridge.parse_resources_payload(payload)

    generation_cfg = cfg.get("generation") or {}
    seed = payload.get("seed", generation_cfg.get("seed"))
    schema_version = str(payload.get("schema_version") or cfg.get("schema_version") or "R5")
    schema_path = payload.get("schema_path")
    dry_run = bool(payload.get("dry_run", False))
    plan_only = bool(payload.get("plan_only", False))
    store_canonical = bool(payload.get("store_canonical", True))
    write_batch_size = max(10, int(payload.get("write_batch_size", 250) or 250))
    variants = bool(payload.get("variants", False))
    variant_resources = payload.get("variant_resources")
    denormalize_after = bool(payload.get("denormalize_after", False))
    scenarios = _parse_scenario_specs(payload.get("scenarios"))
        ...
        try:
            ...
            if denorm_types:
                ...
                if payload.get("limit") is not None:
                    denorm_payload["limit"] = payload.get("limit")
