"""FHIR search → MQL compile path for fhir.rps_canonical."""
...
def _require_converter():
    ...
    return FHIRSearchConverter, MissingConfigurationError, ConversionError
...
def build_search_converter(ctx: StrategyContext):
    ...
    FHIRSearchConverter, _, _ = _require_converter()
    ...
    return FHIRSearchConverter(**kwargs)


def _param_token(name: str, value: str, *, modifier: str | None = None, prefix: str | None = None) -> str:
    key = name
    if modifier:
        key = f"{name}:{modifier}"
    if prefix:
...
def parse_fhir_search(value: str) -> tuple[str, str]:
    """
  Parse ``Patient?family=Smith`` or a full FHIR search URL into (resource_type, query_string).
    """
    text = (value or "").strip()
    if not text:
        raise KehrnelError(
            code="INVALID_INPUT",
            status=400,
            message="fhir_search must be a non-empty string",
        )

    if "?" in text and not text.lower().startswith("http"):
        resource_part, query_string = text.split("?", 1)
        resource_type = resource_part.strip().rstrip("/").split("/")[-1]
        return resource_type, query_string
    ...
    if parsed.scheme and parsed.path:
        ...
        if not resource_type:
            raise KehrnelError(
                code="INVALID_INPUT",
                status=400,
                message="Could not determine resource type from fhir_search URL",
                details={"fhir_search": value},
            )
        return resource_type, query_string

    if "?" in text:
        resource_type, query_string = text.split("?", 1)
        return resource_type.strip(), query_string

    raise KehrnelError(
        code="INVALID_INPUT",
        status=400,
        message="fhir_search must include resource type and query string (e.g. Patient?gender=female)",
        details={"fhir_search": value},
    )
...
def normalize_compile_input(query: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if isinstance(fhir_search, str) and fhir_search.strip():
        rt, qs = parse_fhir_search(fhir_search)
        payload.setdefault("resource_type", rt)
...
async def compile_fhir_query(
    ...
    _, MissingConfigurationError, ConversionError = _require_converter()
...
async def execute_fhir_query(
    ...
    try:
        ...
        rows = _execute_find(collection, mql_filter, limit=limit, skip=skip)
        resolved_filter = resolve_multi_step_mql(collection.database, collection.name, mql_filter)
        total = collection.count_documents(resolved_filter)
    finally:
...
def fhir_list_search_params(ctx: StrategyContext, payload: dict[str, Any] | None) -> dict[str, Any]:
    ...
    _, MissingConfigurationError, _ = _require_converter()
    converter = build_search_converter(ctx)
    try:
