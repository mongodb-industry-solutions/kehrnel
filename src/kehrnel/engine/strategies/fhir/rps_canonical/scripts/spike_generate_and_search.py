...
Standalone script for ``fhir.rps_canonical`` (not imported by the strategy runtime).
...
def main() -> int:
    parser = argparse.ArgumentParser(description="FHIR gen + mql MongoDB spike for fhir.rps_canonical")
    ...
    print(f"[spike] pack={_PACK_ROOT.name} MongoDB {args.uri} db={args.db} seed={args.seed}")
