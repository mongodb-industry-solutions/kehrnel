...
class PersistenceLayout(BaseModel):
    model_config = ConfigDict(extra="ignore")
    name: str = "default"
...
def load_layout_from_file(path: Union[str, Path]) -> PersistenceLayout:
    path = Path(path)
    with path.open("r", encoding="utf-8") as fp:
        data = json.load(fp)
    return PersistenceLayout(**data)


def load_layout_from_json(data: Union[str, Dict[str, Any]]) -> PersistenceLayout:
    if isinstance(data, str):
        data = json.loads(data)
    return PersistenceLayout(**data)


def get_default_layout() -> PersistenceLayout:
