...
from mapper.handlers.common import HandlerCommon as HC
...
class XMLHandler(SourceHandler):
    ...
    def extract_value(self, src: etree._Element, rule: Any) -> Any:
        # 0) template (Jinja with xpath helper)
        if isinstance(rule, dict) and "template" in rule:
            def _xp(expr: str):
                res = src.xpath(expr, namespaces=self.namespaces)
                if not res: return ""
                if len(res) == 1:
                    it = res[0]
                    return it.text if isinstance(it, etree._Element) else str(it)
                # join node-set into strings
                return " ".join(it.text if isinstance(it, etree._Element) else str(it) for it in res)
            ctx = {"xpath": _xp}
            return HC.render_template(str(rule["template"]), ctx)
